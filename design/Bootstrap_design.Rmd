...
- Changed the old names to the names we are using in maicplus to i.e. intervention_data is no longer used
- We removed dplyr dependence. So, we can no longer use dplyr::bind_rows and dplyr::mutate
- I think the function can be reused for anchored setting. In anchored setting, we might not need to merge the pseudo data i.e. if we use HR from the reported comparator study. This should be discussed more thoroughly though.
- Added couple stop statements for heuristic checks.
...
1. The function feed into boot can be separately written, then bootstrap_XX function can be feed into maic_unanchored/maic_anchored function later

2. Separate function for anchored and unanchored for bootstrapping? This is resolved by using the design in item (1)

3. Issue 23, add vignette for using pkg, merge the pull request and then proceed to work on bootstrap design issue
