...
#'   \item ARM - character or factor, treatment indicato, column name does not have to be 'ARM'. User specify in \code{trt_var_ipd} and \code{trt_var_agd}
...
maic_anchored <- function(weights_object,
  ...
  if (any(is.na(ipd$weights))){
    ...
    warning(
      paste("these usubjid in ipd have no weight in weights_object, and hence excluded from analysis:",
