...
#'
#' @details Format requirements for input \code{ipd} and \code{pseudo_ipd} are to have the following columns
#' \itemize{
...
maic_anchored <- function(weights_object,
  ...
  arms_in_pseudo_ipd <- unique(ipd$ARM)
  if (length(arms_in_ipd) != 2) stop(paste("In anchored case, there should be two arms in ipd, but you have:", paste(arms_in_ipd, collapse = ",")))
  if (length(arms_in_pseudo_ipd) != 2) stop(paste("In anchored case, there should be two arms in pseudo_ipd, but you have:", paste(arms_in_pseudo_ipd, collapse = ",")))
  ...
  if (!all(ipd$USUBJID %in% weights_object$data$USUBJID)) {
    stop(paste("these pts in ipd cannot be found in weights_object",
  ...
  if (endpoint_type == "binary") { # for binary effect measure
    if (any(!c("USUBJID","VALUE") %in% names(ipd))) stop("ipd should have 'USUBJID', 'VALUE' columns at minimum")
  ...
  if (any(is.na(ipd$weights))){
    warning(
      paste("these usubjid in ipd have no weight in weights_object:",
