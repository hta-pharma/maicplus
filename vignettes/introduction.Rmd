...
In order to solve the equation above to estimate $\beta$, we could transform IPD by subtracting the aggregate data means. Then $\bar{x}_{agg}$ would equal 0 and the equation would simplify. This is why IPD is centered in the preprocessing step in `maicplus`.
