...
match_res <- estimate_weights(ipd_centered, centered_colnames = centered_colnames)
match_res$ess
...
There is also another option of printing weights using ggplot. 

```{r}
plot_weights2(match_res, print_caption = TRUE, width = 80)
```
...
We first need to combine internal IPD data with pseudo comparator IPD.
...
internal_ipd <- match_res$data

combined_data_tte <- merge_two_data(pseudo_ipd, internal_ipd, internal_time_name = "TIME", internal_event_name = "EVENT")
...
```{r}
km_plot2(combined_data_tte, trt = "A", trt_ext = "B")
```
...
Along with the hazard ratios, we can also find median survival time using `medSurv_make` function. Then, `report_table` function nicely combines the information together and create a result table.
...
```{r, results = 'hide'}
set.seed(1)
HR_bootstraps <- boot(data = internal_ipd,
...
fit_unanchored <- maic_tte_unanchored(external = pseudo_ipd, internal = internal_ipd, internal_time_name = "TIME", internal_event_name = "EVENT", time_scale = "month", endpoint_name = "OS", transform = "log")
```
...
combined_data_binary <- merge_two_data(comparator_binary, internal_ipd, internal_response_name = "RESPONSE")
